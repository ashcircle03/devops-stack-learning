...
def metrics():
    """프로메테우스 메트릭 엔드포인트"""
    # 하트비트 업데이트
    HEARTBEAT_TIMESTAMP.set(time_module.time())
...
def test_error():
    ...
    logger.error("Test error triggered via /test-error endpoint")
    return {"status": "error", "message": "Test error generated"}, 500

@app.route('/test-crash')
def test_crash():
    """테스트용 크래시 시뮬레이션"""
    ERROR_COUNT.labels(error_type='crash_simulation').inc()
    logger.critical("Crash simulation triggered")
